...
CREATE INDEX idx_places_subscribed ON places(is_subscribed);

-- Composite Indexes (for common combined filters)
CREATE INDEX idx_places_city_type ON places(city, type);
CREATE INDEX idx_places_district_type ON places(district, type);
...
CREATE INDEX idx_places_vibe ON places USING GIN(vibe_descriptor);

-- Filtered Index: Active places only (significant performance improvement)
CREATE INDEX idx_places_active_search ON places(search_vector)
